...
def run_simulation(conf):
    try:
        ...
        # logical and physical layout of nodes, respectively
        if conf["topology"] in ["chain", "ring", "grid"]:
            ...
            if os.path.isdir("topo"):
                mangle = (
                    f"num_nodes={conf['num_nodes']}.size={conf['size']}."
                    f"threshold={conf['threshold']}.seed={conf['seed']}"
                )
                topography.export(
                    f"topo/{mangle}-nodes.dat", f"topo/{mangle}-edges.dat"
                )
        ...
        network_factory = QNetwork(
            ...
            qerr_model=FibreLossModel(
                p_loss_init=conf["p_loss_init"], p_loss_length=conf["p_loss_length"]
            ),
        )

        qrepeater_factory = QRepeater(
            dephase_rate=conf["dephase_rate"],
            ...
            if "single" in conf["app"]:
                ...
                if "all" in conf["app"]:
                    for u in range(topology.num_nodes):
                        for v in range(u):
                            pairs.append(
                                [topology.get_name_by_id(u), topology.get_name_by_id(v)]
                            )
        ...
        for node in nodes:
            subprotocol = SwapProtocol(
                name=f"Swap_{node.name}", node=node, oracle=oracle
            )
