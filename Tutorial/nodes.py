...
class DiffusionNode(Node):
    ...
    def __init__(self):
        super().__init__(
            f"DN#{DiffusionNode.counter}", port_names=["in_port", "out_port"]
        )
    ...
    def _handle_message(self, message):
        ...
        if message.items[0] == self.name:
            print(
                f"{ns.sim_time():.1f}: {self.name} message received on final destination: {message.items[1]}"
            )
        else:
            ...
            print(
                f"{ns.sim_time():.1f}: {self.name} forwarding message: {message.items[1]}"
            )
...
class ClassicalConnection(Connection):
    ...
    def __init__(self, length):
        ...
        self.add_subcomponent(
            ClassicalChannel(
                f"Channel_A2B", length=length, models={"delay_model": FibreDelayModel()}
            )
        )
        ...
        self.add_subcomponent(
            ClassicalChannel(
                f"Channel_B2A", length=length, models={"delay_model": FibreDelayModel()}
            )
        )
        self.add_ports(["A2", "B2"])
        self.ports["A2"].forward_input(self.subcomponents["Channel_B2A"].ports["send"])
        self.subcomponents["Channel_B2A"].ports["recv"].forward_output(self.ports["B2"])
...
class MessageGenerator(pydynaa.Entity):
    ...
    def __init__(self, nodes, period):
        ...
        self.period = period
        self._wait(
            pydynaa.EventHandler(self._send),
            entity=self,
            event_type=MessageGenerator.evtype_newmessage,
        )
    ...
    def _send(self, event):
        ...
        print(
            f"{ns.sim_time():.1f}: new message scheduled to be sent "
