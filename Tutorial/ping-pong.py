...
for i in range(num_nodes - 1):
    channels_fwd.append(QuantumChannel(name="conn{}->{}".format(i, i+1),
                        length=distance,
                        models={"delay_model": delay_model}))
    channels_bck.append(QuantumChannel(name="conn{}<-{}".format(i+1, i),
                        length=distance,
                        models={"delay_model": delay_model}))
...
for i in range(num_nodes - 1):
    ...
    protocols_fwd.append(PingPongProtocol(
        nodes[i],
    ...
    protocols_bck.append(PingPongProtocol(
        nodes[i+1],
