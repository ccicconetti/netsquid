...
class ReceivingNode(pydynaa.Entity):
    ...
    def __init__(self):
        ...
        self.qmemory = QuantumMemory(
            "NodeMemory", num_positions=1, memory_noise_models=[noise_model]
        )
        self._wait(
    ...
    def _handle_input_qubit(self, event):
        if self.name == "RingNode#0":
            ...
            print(
                f"{ns.sim_time():.1f}: {self.name} received qubit, "
...
qchannels = [
    QuantumChannel(
        "qchannel0", length=length / 2, models={"delay_model": FibreDelayModel()}
    ),
    QuantumChannel(
        "qchannel0", length=length, models={"delay_model": FibreDelayModel()}
    ),
]
for i in [0, 1]:
